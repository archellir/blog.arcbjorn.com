...
async function processMermaidDiagrams() {
  const mermaidBlocks = [...document.querySelectorAll('pre code')]
    ...
    .filter(({ code }) => /^(graph |flowchart|sequenceDiagram|classDiagram)/.test(code))
  ...
  try {
    ...
    mermaidBlocks.forEach(({ pre, code, index }) => {
      ...
      container.style.cssText = 'opacity: 0; transition: opacity 0.3s ease-in-out';
...
function addCopyButtonsToCodeBlocks() {
  document.querySelectorAll('.markdown-body pre:not(.mermaid)').forEach(block => {
